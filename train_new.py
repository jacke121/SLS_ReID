...
# train_loader = {x: torch.utils.data.DataLoader(image_datasets[x], batch_size=opt.batchsize,
...
val_dataset=Market1501(data_dir, data_transforms['val'], "train")
val_loader = dataloader.DataLoader(val_dataset,shuffle=True,batch_size=opt.batchsize,num_workers=0)
...
def train_model(model, criterion, optimizer, scheduler, num_epochs=25):
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            epoch_loss = float(running_loss) / m_dataset.__len__()
            # epoch_acc = float(running_corrects) / m_dataset.__lesn__()
            y_loss[phase].append(epoch_loss)
            y_err[phase].append(1.0 - (running_corrects/(step+1)))
            # deep copy the model
            if phase == 'val':
...
if __name__ == "__main__":
    state_dict = torch.load(r"E:\github\SLS_ReID\models\model\ft_ResNet50\net_11_0.7107.pth")
