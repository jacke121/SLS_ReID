...
class MyTimer(object):
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        print('{} cost time:{time:.3f}s'.format( self.title,time=time.time() - self.t0))
...
class LinearBottleneck(nn.Module):
    def __init__(self,bott_index,stage, inplanes, outplanes, stride=1, t=1, activation=nn.ReLU6):
        super(LinearBottleneck, self).__init__()
        self.conv1 = nn.Conv2d(inplanes, inplanes * t, kernel_size=1, bias=False)
        self.bn1 = nn.BatchNorm2d(inplanes * t)
        self.stage=stage
        self.bott_index=bott_index
        self.conv3 = nn.Conv2d(inplanes * t, outplanes, kernel_size=1, bias=False)
        self.bn3 = nn.BatchNorm2d(outplanes)
        # need_conv2=False
        # if self.bott_index == 3 and self.stage != 3:
        #     need_conv2=True
        # elif self.bott_index in [2, 3, 4, 5] and self.stage not in[1,2]:
        #     need_conv2 = True
        # elif self.bott_index ==1 and self.stage != 1:
        #     need_conv2 = True
        # if need_conv2:
...
class MobileNet2(nn.Module):
    ...
    def _make_stage(self,bott_index, inplanes, outplanes, n, stride, t, stage):
        modules = OrderedDict()
        stage_name = "LinearBottleneck{}".format(stage)

        # First module is the only one utilizing stride
        first_module = LinearBottleneck(bott_index,stage,inplanes=inplanes, outplanes=outplanes, stride=stride, t=t,
        ...
        for i in range(n - 1):
            ...
            module = LinearBottleneck(bott_index,stage,inplanes=outplanes, outplanes=outplanes, stride=1, t=6,
                                      activation=self.activation_type)
            modules[name] = module
    ...
    def _make_bottlenecks(self):
        ...
        bottleneck1 = self._make_stage(bott_index,inplanes=self.c[0], outplanes=self.c[1], n=self.n[1], stride=self.s[1], t=1,
                                       stage=0)
        ...
        for i in range(1, len(self.c) - 1):
            ...
            module = self._make_stage(bott_index,inplanes=self.c[i], outplanes=self.c[i + 1], n=self.n[i + 1],
                                      stride=self.s[i + 1],
                                      t=self.t, stage=i)
            modules[name] = module
    ...
    def forward(self, x):
        ...
        x = self.bn1(x)
        x = self.activation(x)
        with MyTimer("bottlenecks") as t:
            x = self.bottlenecks(x)
        with MyTimer("conv_last") as t:
            x = self.conv_last(x)
            x = self.bn_last(x)
            x = self.activation(x)
...
if __name__ == "__main__":
    ...
    # print(model2)
    model3 = MobileNet2(in_channels=3, num_classes=10)
    # print(model3)
